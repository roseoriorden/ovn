 ...
 *
 * Returns true if the patch port is still missing from 'src'. */
static bool
                  ...
                  struct shash *existing_ports)
{
    ...
    if (ovs_idl_txn) {
        ...
        const struct smap if_options =
            SMAP_CONST1(&if_options, "peer", dst_name);
...
/* Removes 'port' from the bridge it belongs to.  The port is only removed if
 * 'ovs_idl_txn' is non-NULL.
 *
 * Returns true if 'port' is still present in the database. */
static bool
remove_port(struct ovsdb_idl_txn *ovs_idl_txn,
 ...
 *
 * Returns true if any of those patch ports is still missing from the
 * database. */
static bool
                            ...
                            bool log_missing_bridge)
{
    ...
    SBREC_PORT_BINDING_FOR_EACH_EQUAL (binding, target,
        ...
        missing_ports |= create_patch_port(ovs_idl_txn, patch_port_id,
 ...
 *
 * Returns true if any of the required patch ports is still missing from the
 * database. */
static bool
...
/* Adds to the local OVS database the patch ports required by the localnet and
 * l2gateway ports that are local to this chassis and removes the ones that
 * are no longer needed.  The database is only updated if 'ovs_idl_txn' is
 * non-NULL.
 *
 * Returns true if the database doesn't reflect the required set of patch
 * ports yet.  Until it does, the physical flows that steer traffic through
 * those ports cannot be installed, so the caller must not consider the
 * current configuration to be applied. */
bool
          ...
          const struct hmap *local_datapaths)
{
     ...
     * should be there. */
    bool out_of_sync = add_bridge_mappings(ovs_idl_txn,
                                           sbrec_port_binding_by_type,
    ...
    SHASH_FOR_EACH_SAFE (port_node, &existing_ports) {
        ...
        if (!daemon_started_recently()) {
            out_of_sync |= remove_port(ovs_idl_txn, bridge_table, port);
        }
    }
